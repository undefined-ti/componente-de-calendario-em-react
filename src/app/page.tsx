...
export default function Home() {
  ...
  const handleMouseDown = (event: any, isCheckIn: any, reservationIndex: any) => {
    ...
    const handleMouseMove = (event: any) => {
      ...
      if (isCheckIn) {
        const isDateAvable = checkDate(newDay, Math.max(checkOutIndex, newDay), reservation.IdReserva, reservation.IdImovel);
  ...
  const handleDayDrop = (event: any, day: any) => {
    ...
    //Id do quarto que será criado o hospede.
    const idImovelNew = Math.ceil((event.clientY / 60) - 2);

    setReservations(reservation => reservation.map((_reservaton) => {
      if (_reservaton.IdReserva === id && idImovel !== idImovelNew) {
        return {
          ..._reservaton,
          IdImovel: idImovelNew
        }
      }
      return _reservaton
    }))
    ...
    const isDateAvable = checkDate(newCheckIn, newCheckOut, id, idImovelNew);
  ...
  const checkDate = (checkIn: any, checkOut: any, NewReservationId: any, idImovelNew: any) => {
    ...
    for (const reservation of reservations) {
      ...
      if (reservation.IdImovel === idImovelNew && reservation.IdReserva !== NewReservationId) {

        const reservationCheckInIndex = (datasIntervalo.findIndex((date: any) => date.getTime() === reservationCheckIn.getTime()));
        const reservationCheckOutIndex = (datasIntervalo.findIndex((date: any) => date.getTime() === reservationCheckOut.getTime()));

        // O novo CheckIn pertence ao período desta reserva?
        if (newCheckIn >= reservationCheckInIndex && newCheckIn <= reservationCheckOutIndex) return false

        if (newCheckOut >= reservationCheckInIndex && newCheckOut <= reservationCheckOutIndex) return false
      }
    }
    return true
  }
